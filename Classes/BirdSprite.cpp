...
void BirdSprite::idle() {
	if (changeState(ACTION_STATE_IDLE)) {
		CCLOG("swingAction count %d idleAction count %d", swingAction->getReferenceCount(), idleAction->getReferenceCount());
...
void BirdSprite::fly() {	
	if(changeState(ACTION_STATE_FLY)) {
		CCLOG("swingAction count %d", swingAction->getReferenceCount());
